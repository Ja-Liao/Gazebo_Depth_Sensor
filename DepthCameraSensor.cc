...
class gz::sensors::DepthCameraSensorPrivate
{
  ...
  /// \brief publisher to publish point cloud
  public: transport::Node::Publisher pointPub;
  // ---- Direct ROS (like CameraSensor) ----
  public: std::shared_ptr<rclcpp::Node> directRosNode;
  public: std::string directRosNodeName = "gz_depth_direct";
  public: std::shared_ptr<rclcpp::Publisher<sensor_msgs::msg::Image>> depthImagePub;
...
DepthCameraSensor::~DepthCameraSensor()
{
  ...
  // Edit
  // ---- Release Direct ROS resources ----
  if (this->dataPtr->directRosNode != nullptr)
  {
    this->dataPtr->depthImagePub.reset();
...
bool DepthCameraSensor::Load(const sdf::Sensor &_sdf)
{
  ...
  this->dataPtr->sceneChangeConnection =
    ...
    // ---- Direct ROS setup (mirror CameraSensor) ----
  {
    std::cout << "DepthCamera [" << this->Name() << "] getting direct ROS node" << std::endl;
    this->dataPtr->directRosNode =
        DirectRosNode::GetDirectROSNode(this->dataPtr->directRosNodeName, this->dataPtr.get());
    if (!this->dataPtr->directRosNode)
    {
      gzerr << "Failed creating direct ROS node for DepthCamera sensor ["
            << this->Name() << "]" << std::endl;
      return false;
    }

    // If no topic provided, default to a sensible raw depth topic
    if (this->Topic().empty())
      this->SetTopic("/camera/depth/image_raw");

    if (!this->Topic().empty())
    {
      rclcpp::QoS qos(1);
      this->dataPtr->depthImagePub =
          this->dataPtr->directRosNode->create_publisher<sensor_msgs::msg::Image>(
              this->Topic(), qos);
      gzdbg << "Depth images for [" << this->Name()
            << "] advertised on ROS topic [" << this->Topic() << "]" << std::endl;
    }
  }
                    ...
                    const std::string &_format)
{
  ...
  //Edit
    // ---- Direct ROS publish: 32FC1 depth in meters ----
  if (this->dataPtr->depthImagePub)
  {
    sensor_msgs::msg::Image ros_img;
    ros_img.header.frame_id = this->OpticalFrameId();

    // Use steady_clock to fill builtin_interfaces::Time (simple and consistent with CameraSensor)
    ...
    ros_img.header.stamp.sec = static_cast<int32_t>(sec.count());
    ros_img.header.stamp.nanosec = static_cast<uint32_t>(nsec.count());

    ros_img.encoding = "32FC1";               // float meters
    ros_img.width    = _width;
    ros_img.height   = _height;
    ros_img.step     = _width * sizeof(float);

    const size_t bytes = static_cast<size_t>(_width) * _height * sizeof(float);
    ros_img.data.resize(bytes);
    std::memcpy(ros_img.data.data(), this->dataPtr->depthBuffer, bytes);

    this->dataPtr->depthImagePub->publish(ros_img);
  }
